...
import { getUserRoles, canHaveMultipleRoles, generateDefaultPassword } from '../../utils/helpers';
  ...
  useEffect(() => {
    if (user) {
      ...
      setFormData({
        ...
        rolePasswords: {} // Never pre-fill passwords for existing users
      });
    } else {
      // New user - auto-generate password for default role
      const defaultRole = USER_ROLES.CLIENT;
      setFormData({
        firstName: '',
        middleName: '',
        lastName: '',
        ...
        branchId: '',
        rolePasswords: {
          [defaultRole]: generateDefaultPassword(defaultRole)
        }
      });
    }
  }, [user]);
  ...
  const handleRoleToggle = (role) => {
    setFormData(prev => {
      ...
      if (role === USER_ROLES.CLIENT) {
        ...
        // Auto-generate password for client role
        newRolePasswords[USER_ROLES.CLIENT] = rolePasswords[USER_ROLES.CLIENT] || generateDefaultPassword(USER_ROLES.CLIENT);
      ...
      if (currentRoles.includes(USER_ROLES.CLIENT)) {
        const newRolePasswords = {};
        // Auto-generate password for the new role
        newRolePasswords[role] = rolePasswords[role] || generateDefaultPassword(role);
      ...
      if (currentRoles.includes(role)) {
        ...
        // Add role and auto-generate password
        const newRolePasswords = { ...rolePasswords };
        newRolePasswords[role] = generateDefaultPassword(role);
        return { ...prev, roles: [...currentRoles, role], rolePasswords: newRolePasswords };
      }
    });
  };
