...
const OverallInventoryControllerLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await login(email, password, USER_ROLES.OVERALL_INVENTORY_CONTROLLER);
      navigate(ROUTES.OVERALL_INVENTORY_DASHBOARD);
    } catch (error) {
        ...
        <div className="max-w-md w-full space-y-8">
          <div className="text-center">
            <h2 className="mt-6 text-center text-3xl font-extrabold text-[#160B53]">
              Overall Inventory Controller Login
            </h2>
            <p className="mt-2 text-center text-sm text-gray-600">
              Sign in to access inventory across all branches
            </p>
          </div>
...
export default OverallInventoryControllerLogin;
